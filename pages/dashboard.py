...
def search_names(n_clicks, search_name):
    ...
    df_names_match = df_names[df_names.loc[:,DataSchema.NAME].str.contains(search_name.casefold().strip(), regex=False)]
...
def update_initial_wage_input(dropdown_value, input_value, years, COMPENSATION_TYPE, intial_wage_title):
    min_year = years[0]
...
def update_figures(initial_wage, df_combined_filtered, n_clicks, real_wages_scale_switch_value, years, df_traces_in_real_wages, df_traces_in_projected_wages, fig_projected_wages, fig_real_wages, fig_lollipop, COMPENSATION_TYPE):
    ...
    if (df_traces_in_projected_wages is not None):
        df_traces_in_projected_wages = pd.read_json(df_traces_in_projected_wages, orient='split')
    ...
    if trigger_id == 'real-wages-scale-switch':
        ...
        return df_traces_in_real_wages.to_json(orient="split"), df_traces_in_projected_wages.to_json(orient="split"), fig_projected_wages, fig_real_wages, fig_lollipop, lollipop_chart_title
