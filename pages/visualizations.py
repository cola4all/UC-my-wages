...
APP_PATH = os.path.split(str(pathlib.Path(__file__).parent.resolve()))[
    0
]  # the 0th element of os path split moves back one subfolder
...
LIVING_WAGE_DATA_PATH = os.path.join(
    APP_PATH, "assets", "living_wage_by_university.csv"
)
...
UC_list = [
    ...
    "UC Riverside",
    "UC Davis",
    "UC Los<br>Angeles<br>",
    "UC San Diego",
    "UC Berkeley",
    "UC Irvine",
    "UC Santa<br>Barbara<br>",
    "UC Santa<br>Cruz<br>",
    "UC San<br>Francisco<br>",
]
UC_list.reverse()
print(time.time() - t0)
                        ...
                        dbc.Row(
                            [
                                ...
                                dbc.Popover(
                                    [
                                        dbc.PopoverBody(
                                            "GSR = Graduate Student Researchers\nASE = Academic Student Employees"
                                        ),
                                    ],
                                    target="GSR-ASE-info",
                                    placement="bottom",
                                ...
                                dbc.Popover(
                                    [
                                        dbc.PopoverBody(
                                            "Wage plots the actual monthly wage for each step as stimulated in the tentative agreement.",
                                            style={"padding-bottom": ".25rem"},
                                        ),
                                        dbc.PopoverBody(
                                            "Rent Burden plots the local monthly rent (determined as fair market rate from HUD dataset) as a percentage of monthly rent.",
                                            style={"padding-top": ".25rem"},
                                        ),
                                    ],
                                    target="wages-type-info",
                                    placement="bottom",
                                    trigger="legacy",
                                ),
                                dbc.Col(
                                ...
                                dbc.Popover(
                                    [
                                        dbc.PopoverBody(
                                            "These 3 dates correspond to when each raise would take effect, according to the tentative agreement."
                                        ),
                                    ],
                                    target="date-info",
                                    placement="bottom",
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H5(
                            "Does this contract reduce or increase rent burden for you?"
                        ),
                        dcc.Markdown(
...
proposal_y = (
    df_proposal["Position"].str.replace(" ", "<br>", n=1).tolist()
...
if len(proposal_y) < 6:
    ...
    fig_proposal.update_layout(
        height=(len(proposal_y) - 6) * 50 + 400
...
wage_proposal_div = html.Div(
    [
        html.H4(
            "How have the UAW and UC base pay proposals changed over the course of negotiations?"
        ),
        dbc.Row(
            [
                dbc.Col(
                    dcc.Graph(figure=fig_proposal, config={"displayModeBar": False}),
                    xl=9,
                    lg=12,
                ),
                dbc.Col(
...
def update_fig_universities(radio_value, wage_value, date_value):
    ...
    if wage_value == 2:
        fig_universities.add_vrect(x0=0, x1=30, fillcolor="#00A724", opacity=0.2)
        fig_universities.add_vrect(x0=30, x1=50, fillcolor="#FAD000", opacity=0.2)
        fig_universities.add_vrect(x0=50, x1=120, fillcolor="#7F0000", opacity=0.2)

        fig_universities.add_vline(
            x=30, line_width=3, line_dash="dash", line_color="grey"
        )
        fig_universities.add_vline(
            x=50, line_width=3, line_dash="dash", line_color="grey"
        )

        fig_universities.add_annotation(
            yref="paper", x=40, y=1.07, text="Rent<br>Burdened", showarrow=False
        ),
        fig_universities.add_annotation(
            yref="paper", x=80, y=1.07, text="Extreme Rent<br>Burdened", showarrow=False
        ),
    ...
    for school in unique_schools:
            ...
            if "UC " in school:
                if position == "ASE":
                    if df_fmr.loc[df_fmr["School"] == school, "Tier"].iloc[0] == 1:
                        logical_array = (
                            dff_universities["School"]
                            == "UC Tentative<br>Agreement<br>(Tier 1 Campus)<br><br>"
                        )
                    elif df_fmr.loc[df_fmr["School"] == school, "Tier"].iloc[0] == 2:
                        logical_array = (
                            dff_universities["School"]
                            == "UC Tentative<br>Agreement<br>(Tier 2 Campus)<br><br>"
                        )
                elif position == "GSR":
                    logical_array = (
                        dff_universities["School"] == "UC Tentative<br>Agreement<br>"
                    )
            else:
                logical_array = dff_universities["School"] == school
            # switch WAGE COLUMN from wage to rent/wage
            dff_universities[WAGE_COLUMN_ADJUSTED] = (
                df_fmr.loc[df_fmr["School"] == school, LIVING_WAGE_COLUMN].iloc[0]
                / dff_universities[WAGE_COLUMN]
                * 100
            )

        if sum(logical_array) > 1:  # only plot line if more than 1 data point available
            fig_universities.add_trace(
                go.Scatter(
                    showlegend=False,
                    mode="lines",
                    ...
                    hoverinfo="skip",
                    line=dict(
                        color=dff_universities.loc[logical_array, "Line Color"].iloc[0],
                        width=2,
                    ),
                )
            )
        ...
        customdata = np.stack(
            (
                dff_universities.loc[minimum_logical_array, "Step"],
                dff_universities.loc[minimum_logical_array, "Position"],
            ),
            axis=-1,
        )
        fig_universities.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    color=dff_universities.loc[
                        minimum_logical_array, "Marker Color"
                    ].iloc[0],
                    size=8,
                ),
            )
        ...
        non_minimum_logical_array = logical_array & (
            dff_universities["Min or Max"] == 0
        )
        customdata = np.stack(
            (
                dff_universities.loc[non_minimum_logical_array, "Step"],
                dff_universities.loc[non_minimum_logical_array, "Position"],
            ),
            axis=-1,
        )
        if (
            sum(non_minimum_logical_array) > 0
        ):  # only plot line if more than 0 data point available
            fig_universities.add_trace(
                go.Scatter(
                    showlegend=False,
                    mode="markers",
                    marker_symbol="circle",
                    x=dff_universities.loc[
                        non_minimum_logical_array, WAGE_COLUMN_ADJUSTED
                    ],
                    ...
                    marker=dict(
                        color=dff_universities.loc[
                            non_minimum_logical_array, "Marker Color"
                        ].iloc[0],
                        size=6,
                        opacity=0.5,
                    ),
    ...
    if wage_value == 1:
        fig_universities.update_xaxes(
            range=[0, 5000], tickprefix="$", title=dict(text="Monthly Wage (USD)")
        )
    elif wage_value == 2:
        fig_universities.update_xaxes(
            range=[0, 120],
            ticksuffix="%",
            title=dict(text="Rent Burden (Rent as a % of Monthly Wage)"),
        )
    ...
    if len(unique_schools) < 6:
        ...
        fig_universities.update_layout(
            height=(len(unique_schools) - 6) * 50 + 400
        )  # increase height by 30px for each additional person past 5
